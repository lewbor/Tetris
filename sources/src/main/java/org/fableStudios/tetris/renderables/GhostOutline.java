...
import org.fableStudios.tetris.BaseTetrisPiece;
    ...
    public void render(Graphics g) {
        ...
        for (i = 0; i < grid.length; i++)
            for (j = 0; j < grid[i].length; j++)
                if (grid[i][j].hasBorder()) {
                    x = TetrisPanel.getX(j + this.x + this.xOffset);
                    y = TetrisPanel.getY(Main.panel.getBoardHeight() - this.y - i - this.yOffset - 1);
                    if (grid[i][j].left)
                        g.drawLine(x, y + TetrisPanel.squareHeight() - 1, x, y);
                    if (grid[i][j].top)
                        g.drawLine(x, y, x + TetrisPanel.squareWidth() - 1, y);
                    if (grid[i][j].bottom)
